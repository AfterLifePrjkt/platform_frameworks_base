 ...
 */
open class LandscapeBatteryL(private val context: Context, frameColor: Int) : Drawable() {
    ...
    private fun percentTypeface(): Typeface {
        var typefaceBuilder: Typeface.Builder? = null
        try {
            if (typefaceBuilder == null) {
                typefaceBuilder = Typeface.Builder(context.assets, "SanFranciscoText-Semibold.otf")
            }
            return typefaceBuilder.build() ?: Typeface.create("sans-serif-condensed", Typeface.BOLD)
        } catch (e: Exception) {
    ...
    override fun draw(c: Canvas) {
        ...
        chargingAlphaPaint.color = if (customBlendColor && chargingColor != BLACK) chargingColor else chargingParseColor
        chargingPaint.color = if (customBlendColor && chargingColor != BLACK) chargingColor else chargingParseColor
        powerSavePaint.color = if (customBlendColor && powerSaveColor != BLACK) powerSaveColor else powerSaveParseColor
        powerSaveFillPaint.color = if (customBlendColor && powerSaveFillColor != BLACK) powerSaveFillColor else powerSaveParseColor
        ...
        if (charging) {
            ...
            customFillAlphaPaint.alpha = 85
            c.drawPath(scaledPerimeter, if (customBlendColor && customFillColor != BLACK) customFillAlphaPaint else scaledPerimeterPaint)
        }
        ...
        if (dualTone) {
            ...
            if (charging) {
                ...
                if (customFillColor != BLACK && customFillGradColor != BLACK) LinearGradient(
                    ...
                    Shader.TileMode.CLAMP) else null
                c.clipOutPath(scaledfillOutline)
                c.drawPath(unifiedPath, if (customBlendColor && customFillColor != BLACK) customFillPaint else fillPaint)
                fillPaint.color = levelColor
            }
        ...
        if (charging || batteryLevel <= Companion.CRITICAL_LEVEL) {
            ...
            textQsPaint.color = Utils.getColorAttrDefaultColor(context, android.R.attr.textColorPrimaryInverse)
            if (isRotation) {
                c.rotate(180f, pctX, pctY * 0.63f)
            }
            c.drawText(batteryLevel.toString(), pctX, pctY, if (isQsPercent) textQsPaint else textPaint)
    ...
    private fun loadPaths() {
        ...
        val boltPathString = context.resources.getString(
                R.string.config_landscapeBatteryBoltPathL)
        boltPath.set(PathParser.createPathFromPathData(boltPathString))

        val plusPathString = context.resources.getString(
                R.string.config_landscapeBatteryPowersavePathL)
    ...
    companion object {
        private const val TAG = "LandscapeBatteryL"
